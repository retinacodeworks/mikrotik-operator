...
//+kubebuilder:rbac:groups=routeros.mikrotik.com,resources=vlans,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=routeros.mikrotik.com,resources=vlans/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=routeros.mikrotik.com,resources=vlans/finalizers,verbs=update
...
func (r *VlanReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	var vlan *routerosv1alpha1.Vlan
	if err := r.Get(ctx, req.NamespacedName, vlan); err != nil {
		if errors.IsNotFound(err) {
			logger.Error(err, "vlan resource did not exist")
			return ctrl.Result{}, nil
		} else {
			logger.Error(err, "failed finding vlan resource")
	...
	if err := r.Get(ctx, types.NamespacedName{
		Namespace: vlan.Spec.RouterRef.Namespace,
		Name:      vlan.Spec.RouterRef.Name,
	}, &router); err != nil {
		if errors.IsNotFound(err) {
